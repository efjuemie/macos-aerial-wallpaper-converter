...
普通用户无需 clone 源码。发行包发布在本仓库的 [Releases](https://github.com/efjuemie/macos-aerial-wallpaper-converter/releases) 页面，请下载与设备匹配的 Apple Silicon DMG 或 ZIP。`v0.8.4`（build 12）是 Finder 拖入载入状态修复预发布版本；它不会宣称已经在所有 Mac 上解决“进入桌面后被压扁”。
...
桌面保持静态是 macOS 的正常现象；若出现黑屏、冻结或第二次不播放，请在应用中恢复备份。只有受影响设备完成上述 5 次循环且圆形、方形和人物比例均正常，才能把该设备视为回归通过；在此之前不要把 v0.8.4 当作完整桌面变形修复。

## v0.8.4（build 12）Finder 拖入载入修复预发布说明

本版本针对“从 Finder 拖入后路径出现在输入框，但视频并未真正载入，处理按钮仍灰置”的回归。路径输入框仍可直接输入或粘贴本地路径并按回车；拖入文件时请将文件松开到独立的“将本地视频拖到这里”区域，应用会自动开始读取。也可以在路径框旁点击“载入”，显式提交当前路径。

- 拖放区域与可编辑路径框分离，避免 macOS 将 Finder 文件拖放仅当作文本填入输入框。
- 成功载入后会显示视频文件名；无法识别或读取时会显示错误，而不是静默保持灰置。
- 视频载入后如果修改了路径，当前视频不会被当作新路径的结果使用；请重新点击“载入”或按回车，确认目标文件完成读取后再处理。
- 处理入口仍保留目标文件、环境检查、几何和 temporal sample groups 等全部安全校验；本修复不会通过删除校验来强行启用按钮。
- 自动测试覆盖输入就绪状态、路径解析、拖放桥接类型和读取任务取消；Finder 真机拖放、网络链接提示和受影响设备上的锁屏→解锁流程仍需人工验证。
...
按钮下方会显示具体原因。请先确认视频已真正载入、目标动态壁纸已从“系统设置 → 壁纸”下载并选中，且必需环境检查没有失败；读取视频期间会显示“正在读取视频信息…”，目标不可用时会提示重新选择目标。拖入 Finder 文件时请松开到独立拖放区，不要只把文件放到可编辑路径框；也可以点击路径框旁的“载入”或按回车显式提交。若输入框路径在视频载入后被修改，必须重新载入后才能处理。读取失败会在输入区域保留错误信息。路径输入支持本地路径、`~/...`、`file://` 和成对引号；网络链接不支持直接下载，请先保存到本地。连续替换输入时应用会以最后一次输入为准，无需通过重启应用清除读取状态。

### 为什么桌面不动或锁屏黑屏？

请在系统设置中重新点击对应动态壁纸并连续测试 5 次锁屏→解锁，观察解锁后立即和约 5 秒后的桌面画面。若仍异常，先导出“几何诊断报告”，再在应用中恢复备份；不要运行常驻杀进程脚本。v0.8.4 仍是 Finder 拖入载入与此前诊断/裁剪交互预发布版本，尚不能据此宣称所有设备的桌面变形问题已经解决。
...
当前源码版本为 **0.8.4（build 12，Finder 拖入载入修复预发布）**，完整记录见 [CHANGELOG.md](CHANGELOG.md)。
